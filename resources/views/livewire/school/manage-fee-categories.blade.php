<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <div>
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white">Fee Categories</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Manage fee heads like Tuition, Library, Sports</p>
            </div>
        ...
        <div class="flex flex-col sm:flex-row gap-4 mb-6">
            <input wire:model.live="search" type="text" placeholder="Search categories..." class="flex-1 px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-emerald-500 dark:text-white">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                <div class="mb-4">
                    ...
                    @if($category->description)<p class="text-sm text-gray-500 dark:text-gray-400 mt-2">{{ $category->description }}</p>@endif
                </div>
                <div class="flex gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
                    <button wire:click="toggleStatus('{{ $category->id }}')" wire:loading.attr="disabled" wire:target="toggleStatus('{{ $category->id }}')" class="flex-1 px-3 py-2 {{ $category->is_active ? 'bg-gray-50 hover:bg-gray-100 text-gray-600' : 'bg-emerald-50 hover:bg-emerald-100 text-emerald-600' }} font-medium rounded-lg text-sm disabled:opacity-50">
                        <span wire:loading.remove wire:target="toggleStatus('{{ $category->id }}')">{{ $category->is_active ? 'Disable' : 'Enable' }}</span>
                        <svg wire:loading wire:target="toggleStatus('{{ $category->id }}')" class="animate-spin h-4 w-4 mx-auto" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                    </button>
                    <button wire:click="edit('{{ $category->id }}')" wire:loading.attr="disabled" wire:target="edit('{{ $category->id }}')" class="flex-1 px-3 py-2 bg-blue-50 hover:bg-blue-100 text-blue-600 font-medium rounded-lg text-sm disabled:opacity-50">
                        <span wire:loading.remove wire:target="edit('{{ $category->id }}')">Edit</span>
                        <svg wire:loading wire:target="edit('{{ $category->id }}')" class="animate-spin h-4 w-4 mx-auto" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                    </button>
                    <button wire:click="delete('{{ $category->id }}')" wire:loading.attr="disabled" wire:target="delete('{{ $category->id }}')" class="flex-1 px-3 py-2 bg-red-50 hover:bg-red-100 text-red-600 font-medium rounded-lg text-sm disabled:opacity-50">
                        <span wire:loading.remove wire:target="delete('{{ $category->id }}')">Delete</span>
                        <svg wire:loading wire:target="delete('{{ $category->id }}')" class="animate-spin h-4 w-4 mx-auto" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                    </button>
                </div>
            </div>
            @empty
            <div class="col-span-full text-center py-12"><p class="text-gray-500">No categories found</p></div>
            @endforelse
        </div>
        @else
                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase">Description</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            ...
                            <tr>
                                <td class="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{{ $categories->firstItem() + $index }}</td>
                                ...
                                <td class="px-6 py-4"><span class="px-2 py-0.5 text-xs font-medium rounded-full {{ $category->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700' }}">{{ $category->is_active ? 'Active' : 'Inactive' }}</span></td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        <button wire:click="toggleStatus('{{ $category->id }}')" wire:loading.attr="disabled" wire:target="toggleStatus('{{ $category->id }}')" class="px-3 py-1.5 {{ $category->is_active ? 'bg-gray-50 text-gray-600' : 'bg-emerald-50 text-emerald-600' }} font-medium rounded-lg text-sm disabled:opacity-50">
                                            <span wire:loading.remove wire:target="toggleStatus('{{ $category->id }}')">{{ $category->is_active ? 'Disable' : 'Enable' }}</span>
                                            <svg wire:loading wire:target="toggleStatus('{{ $category->id }}')" class="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                        </button>
                                        <button wire:click="edit('{{ $category->id }}')" wire:loading.attr="disabled" wire:target="edit('{{ $category->id }}')" class="px-3 py-1.5 bg-blue-50 text-blue-600 font-medium rounded-lg text-sm disabled:opacity-50">
                                            <span wire:loading.remove wire:target="edit('{{ $category->id }}')">Edit</span>
                                            <svg wire:loading wire:target="edit('{{ $category->id }}')" class="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                        </button>
                                        <button wire:click="delete('{{ $category->id }}')" wire:loading.attr="disabled" wire:target="delete('{{ $category->id }}')" class="px-3 py-1.5 bg-red-50 text-red-600 font-medium rounded-lg text-sm disabled:opacity-50">
                                            <span wire:loading.remove wire:target="delete('{{ $category->id }}')">Delete</span>
                                            <svg wire:loading wire:target="delete('{{ $category->id }}')" class="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                        </button>
                                    </div>
                                </td>
                            </tr>
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto" style="background-color: rgba(0, 0, 0, 0.7);">
            <div class="flex min-h-screen items-center justify-center p-4">
                ...
                <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md p-6">
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-4">{{ $categoryId ? 'Edit Category' : 'Add Category' }}</h3>
                    <div class="space-y-4">
                        <div>
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto" style="background-color: rgba(0, 0, 0, 0.7);">
            <div class="flex min-h-screen items-center justify-center p-4">
                ...
                <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md p-6 text-center">
                    ...
                    <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Delete Category?</h3>
                    <p class="text-gray-500 dark:text-gray-400 mb-6">This will permanently delete the fee category.</p>
